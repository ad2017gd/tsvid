...
export class RendererFFMPEG extends Renderer {
    ...
    private renderVideoElement(editor: Editor, element : VideoElement, count : number) : fffmpeg.FilterSpecification {
        if(element instanceof TextVideoElement) {
            return {
                ...
                outputs: `jsv${count+1}`,
                options: {
                    "x": this.interpKeyframeProp(element.transform, "x"),
                    "y": this.interpKeyframeProp(element.transform, "y"),
                    ...
                    "fontsize": `${element.properties.size} * (${this.interpKeyframeProp(element.transform, "scale")})`,
                    "alpha": this.interpKeyframeProp(element.transform, "opacity"),
    ...
    private interpKeyframeProp(effect : Effect<EffectProperties>, keyframeProp : string) {
        if(typeof (effect.keyframeManager.keyframes[0].properties as any)[keyframeProp] == "number") {
        ...
        } else if (typeof (effect.keyframeManager.keyframes[0].properties as any)[keyframeProp] == "string" ) {
            return effect.keyframeManager.keyframes.slice(0,-1).map((x,i)=>
            {
                let kp = `((n - ${x.timing.frameStart})/${effect.keyframeManager.keyframes[i+1].timing.frameStart - x.timing.frameStart})`;
                return `if(lte(1,${kp}), `
            ...
            effect.keyframeManager.keyframes.slice(0,-1).map((x,i)=>
            {
                let ki = `${(x.properties as any)[keyframeProp]}`;
                return `, ${ki})`
            }
            ).reverse().join("")
            }
